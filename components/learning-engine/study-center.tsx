...
export function StudyCenter() {
  return (
    <>
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-2"><StudyPanel eyebrow="Quick reviews" title="Test your recall"><LinkList items={mockQuickReviews} /></StudyPanel><StudyPanel eyebrow="Saved bookmarks" title="Return to saved references"><LinkList items={mockSavedBookmarks} /></StudyPanel><StudyPanel eyebrow="Recommended lessons" title="Continue learning"><LinkList items={mockRecommendedLessons} /></StudyPanel><StudyPanel eyebrow="Progress snapshot" title="What to focus on next"><p className="text-sm leading-6 text-slate-600">Finish Lesson 2, then complete the Quick Review Assessment. Your mock progress updates only inside each interactive component.</p></StudyPanel></div>
      <div className="mt-6"><StudyPanel eyebrow="Flashcards" title="Key concepts"><FlashcardDeck cards={mockFlashcards} /></StudyPanel></div>
    </>
  );
}
